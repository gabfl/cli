...
function ts() { date +"%s"; }
function tsc() {
    env TZ=":US/Eastern" date -r $1 +"Eastern: %Y-%m-%d %H:%M:%S";
    env TZ="UTC" date -r $1 +"UTC:     %Y-%m-%d %H:%M:%S";
}

# Git commit / pull / push  rapide
#function gcommit() { git add . && git commit -am "$1"; }
function gstatus() { git status -u; }
...
function gpurge() { sudo git checkout .; }
function grm() { git ls-files --deleted -z | xargs -0 git rm ; }

# Resize images
function resize() { sips -Z 1024 "$1"; }
